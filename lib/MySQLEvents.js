...
class MySQLEvents extends EventEmitter {
  constructor(connection, zongJiOptions = {}, systemOptions = {}) {
    super();

    this.connection = connection;
    this.options = zongJiOptions;
    ...
    this.logger = systemOptions.logger || {
      debug: console.log
    };
  }
  ...
  _handleEvent(event) {
    ...
    })).then(() => this.logger.debug('triggers executed'));
  }
  ...
  async start(options = {}) {
    ...
    this.logger.debug('initializing zongji');
  ...
  async stop() {
    if (!this.isStarted) return;
    this.logger.debug('disconnecting from mysql');
  ...
  pause() {
    ...
    this.logger.debug('pausing connection');
  ...
  resume() {
    if (!this.isStarted || !this.isPaused) return;
    this.logger.debug('resuming connection');
